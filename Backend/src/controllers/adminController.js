...
const updateDriverStatus = async (req, res) => {
    ...
    try {
        const driver = await Driver.findByIdAndUpdate(driverId, { suspensionStatus: status }, { new: true });
        if (!driver) {
...
const approveDriver = async (req, res) => {
    ...
    try {
        const driver = await Driver.findByIdAndUpdate(driverId, { approved: true }, { new: true });
        if (!driver) {
            return res.status(404).json({ message: 'Driver not found' });
        }
...
const updateFare = async (req, res) => {
    ...
    if (!['Ride AC', 'Ride Mini', 'Motoride', 'Horse', 'Spiderman', 'Superman'].includes(rideType)) {
...
const removePromotion = async (req, res) => {
    ...
    try {
        ...
        const promotionIndex = fare.promotions.findIndex(promotion => promotion.promotionCode === promotionCode);
...
const updatePromotion = async (req, res) => {
    ...
    try {
        ...
        const promotion = fare.promotions.find(promo => promo.promotionCode === promotionCode);
