...
const addEarnings = async (req, res) => {
  try {
    ...
    return res.status(201).json({ message: 'Earnings added successfully', data: savedEarnings });
  } catch (error) {
    console.error(error); // Log the error for server-side debugging
    return res.status(500).json({ message: 'Server error while adding earnings', error: error.message });
  }
};
...
const getEarningsSummary = async (req, res) => {
  try {
    ...
    if (!driverId || !period) {
      return res.status(400).json({ message: 'Driver ID and period (daily, weekly, monthly) are required' });
    }
    ...
    if (earnings.length === 0) {
      return res.status(404).json({ message: 'No earnings found for the given period' });
    }
    ...
    res.status(500).json({ message: 'Server error while fetching earnings summary' });
  }
};
